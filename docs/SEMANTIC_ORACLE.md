...
The current 40-case corpus has no intentional GAP after v0.12.44: cases 30–32 have exact canonical conditional expectations, and genuine attachment ambiguity in case 39 passes only when the system returns the expected structural clarification without guessing. `ARCHITECTURE_GAP` remains a valid oracle grade for future corpus items whose semantics are not yet defined by the working architecture.
