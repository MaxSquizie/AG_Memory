...
ah-agent --config config/ollama.toml import-dialogue dialogue.json
...
Холодное сохранение/восстановление используется по умолчанию. `--hot-save` и
`--hot-restore` нужны только когда требуется сохранить либо восстановить runtime
excitation и pending impulses. Пример структуры: `data/corpus/example.json`.
