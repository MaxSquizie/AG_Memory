...
class GUIRuntimeControlsV029Tests(unittest.TestCase):
    ...
    def test_ignition_panel_exposes_pacemaker_and_cold_load(self) -> None:
        ...
        self.assertIn("raw_text_import_requested", source)
        self.assertIn("memory_import_requested", source)
        self.assertIn("Загрузить текст", source)
        self.assertIn("Разбирать в факты", source)
        self.assertNotIn("dialogue_import_requested", source)
        main = MAIN.read_text(encoding="utf-8")
        self.assertIn('QDockWidget("Ignition"', main)
        self.assertIn("def _import_corpus", main)
        self.assertIn("def _import_raw_text", main)
        self.assertIn("def _import_dialogue", main)
        self.assertIn("def _export_dialogue", main)
        self.assertIn("def _import_memory", main)
        self.assertIn("Экспорт диалога", main)
        self.assertIn("Импорт диалога", main)
