...
class NamingAwareIntegrationService(_BaseIntegrationService):
    """Consume identity naming and open-event query staging before canonical writes.

    Two source structures deliberately do not belong to ordinary predicate/T
    integration:
    ...
    * ``EventSetQueryCandidate`` asks for matching factual events with an open
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self._query_temporal_normalizer = TemporalNormalizer()
    ...
    def _normalize_query_temporals(
        self,
        result: PerceptionResult,
        context: InteractionContext,
        source_timestamp: datetime | None,
    ) -> PerceptionResult:
        """Resolve query TIME values from the same authoritative turn anchor as facts."""
        ...
        for query in result.queries:
            ...
            for actant in query.actants:
                ...
                if not text:
                    actants.append(actant)
                    continue
                temporal = self._query_temporal_normalizer.normalize(text, anchors)
                if temporal is None:
                    actants.append(actant)
                    continue
                actants.append(replace(actant, temporal=temporal))
                query_changed = True
            queries.append(replace(query, actants=tuple(actants)) if query_changed else query)
            changed = changed or query_changed
        return replace(result, queries=tuple(queries)) if changed else result
